...
public class Log{
	...
	public Log(string user, int task)
	{
		...
		string header = "Time;PieceID;IsSelected;DistanceToTarget;DistanceID;Modality;Translation X;Translation Y;Translation Z;Rotation X;Rotation Y;Rotation Z;Rotation W;Camera X;Camera Y;Camera Z;Error Trans;Error Rot;Tracked Targets";
	...
	public void saveVerbose(int pieceID, bool isSelected, int distanceToTarget, int distanceID, int modality, GameObject piece, Vector3 cameraPosition, float errorTrans, float errorRot, int trackedTargets) //, float distToTarget, GameObject t, Quaternion cameraRotation, float errorTrans, float errorRot)
	{
	...
	//	//if (clients.Count < numberOfClients) return;
        ...
        line += ";" + pieceID + ";" + isSelected + ";" + distanceToTarget + ";" + distanceID + ";" + modality;
